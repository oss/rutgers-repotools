...
# set VARS
    ...
    R_DIR=/limbus/centos/fedora/releases
    P_DATEFILE=FEDORA.LAST_UPDATE
    P_LOG=$P_DIR/FEDORA_LAST_MIRRORED.$DATE
    O_LOG=$P_DIR/FEDORA_LAST_MIRRORED.$YEST_DATE	#O is for OLD
    T_LOG=$P_DIR/FEDORA_MIRROR_IN_PROGRESS.$DATE
    E_LOG=$P_DIR/FEDORA_BROKE_ON.$DATE			#E is for ERROR
    RSYNC_COMMAND="/usr/local/share/rutgers-mirror-scripts/run-mirror-fedora.sh"
...
then
    ...
    if [ "$OUTOFDATE" -eq 0 ]
    then
        echo "Warning: FEDORA mirror out of date by 24 hours, please check logs at http://centos.rutgers.edu/mirror/status " | mail -s "FEDORA MIRROR 24 Hours out of date" $EMAIL
    fi
fi
